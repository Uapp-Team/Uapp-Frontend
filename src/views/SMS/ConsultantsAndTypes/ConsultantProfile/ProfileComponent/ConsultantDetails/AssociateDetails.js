...
const AssociateDetails = ({ id }) => {
  ...
  useEffect(() => {
    if (id !== undefined) {
      ...
      get(`Consultant/Profile/${userId}`).then((res) => {
        setConsultantData(res);
        setContactInfo(res?.consultantContact);
        setEligibility(res?.consultantProfileEligibility);
        console.log("cons data", res);
      ...
      get(`GroupPriceRange/ByConsultant/${userId}`).then((res) => {
        setPriceRangeList(res);
      });

      get(`ConsultantCommissionGroup/Index/${userId}`).then((res) => {
        setCommissionGrouplist(res);
      });
    }
  }, []);
  ...
  const handleUpdateBankDetailsFromProfile = () => {
    history.push(`/consultantBankInformation/${id}`);
  };
