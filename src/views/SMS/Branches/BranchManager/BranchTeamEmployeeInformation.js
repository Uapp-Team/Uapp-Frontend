...
const BranchTeamEmployeeInformation = () => {
  ...
  useEffect(() => {
    get(`BranchTeamEmployee/GetEmployee/${teamId}`).then((res) => {
      setTeamDetails(res);
    });
  ...
  const handleDeleteEmployee = () => {
    ...
    remove(`BranchTeamEmployee/Delete/${delData?.teamEmployeeId}`).then(
      (res) => {
        ...
        {teamDetails.length > 0 ? (
          <div className="table-responsive container mt-3">
            <Table className="table-sm table-bordered">
              ...
              <tbody>
                {teamDetails?.map((details, i) => (
                  <tr key={details?.id} style={{ textAlign: "center" }}>
                    {/* <td>{serialNum + i}</td> */}
                    ...
                    <td>
                      <Button
                        color="primary"
                        className="mr-1"
                        onClick={() => gotoEmployeeProfile(details)}
                      >
