...
const Branch = () => {
  ...
  const employeeProfileImage = useSelector(
    (state) => state?.BranchEmployeeProfileImageReducer?.employeeProfileImage
  );

  const employeeCoverImage = useSelector(
    (state) => state?.BranchEmployeeCoverImageReducer?.employeeCoverImage
  );
  ...
  useEffect(() => {
    get(`BranchEmployee/Index`).then((res) => {
      setBranchEmployee(res);
      setEmpty(true);
    });

    get(`EmployeeTypeDD/ForBranch`).then((res) => {
      setEmp(res);
    });
    ...
    get(`BranchEmployee/Get/${employeeId}`).then((res) => {
      setEmployeeInfo(res);
    ...
    if (empValue == 0) {
      ...
      if (!employeeId) {
        if (pass.length < 6) {
          ...
          Axios.post(`${rootUrl}BranchEmployee/Create`, subdata, config).then(
            (res) => {
        ...
        setButtonStatus(true);
        Axios.put(`${rootUrl}BranchEmployee/Update`, subdata, config).then(
          (res) => {
            setButtonStatus(false);
            if (res?.status === 200 && res?.data?.isSuccess === true) {
              addToast(res?.data?.message, {
  ...
  return (
    <div>
      <BreadCrumb
        title="Branch Employee Information"
        backTo="Branch"
        path={`/branchList`}
      />
         ...
         branchEmployee?.map((bemp, i) => <tr key={bemp.id} style={{ textAlign: "center" }}>
          ...
          <>
            <Form className="mt-5" onSubmit={handleSubmit}>
              ...
              {employeeId ? (
                <Input type="hidden" name="id" id="id" value={employeeId} />
              ) : null}
                    ...
                    {employeeId ? (
                      <div className="mr-2">
                        <Image
                          ...
                          src={
                            rootUrl +
                            employeeInfo?.profileImageMedia?.thumbnailUrl
                          }
                        />
                      </div>
                    ) : null}
              ...
              <FormGroup row className="has-icon-left position-relative">
                ...
                <Col md="4">
                  <div className="d-flex">
                    {employeeInfo?.coverImageMedia?.thumbnailUrl !== null &&
                    employeeId !== undefined ? (
                      <div className="mr-2">
                        <Image
                          ...
                          src={
                            rootUrl +
                            employeeInfo?.coverImageMedia?.thumbnailUrl
                          }
                        />
                      </div>
                    ) : null}
                    ...
                    <div className="ms-2">
                      <BranchCoverImage setText1={setText1} />
