...
import ColumnApplicationAssociate from "../../../TableColumn/ColumnApplicationAssociate.js";
import ConfirmModal from "../../../../../components/modal/ConfirmModal.js";

const AssociateApplication = ({ currentUser }) => {
  const history = useHistory();
  const { addToast } = useToasts();
  const componentRef = useRef();
  const { status, selector, universityId, intake, consultantId } = useParams();
  ...
  useEffect(() => {
    const tableColumnApplicationAssociate = JSON.parse(
      localStorage.getItem("ColumnApplicationAssociate")
    );
    tableColumnApplicationAssociate &&
      setTableData(tableColumnApplicationAssociate);
    !tableColumnApplicationAssociate &&
      localStorage.setItem(
        "ColumnApplicationAssociate",
        JSON.stringify(ColumnApplicationAssociate)
      );
    !tableColumnApplicationAssociate &&
      setTableData(ColumnApplicationAssociate);
  }, []);
  ...
  useEffect(() => {
    get(
      `Application/ByAssociate?page=${currentPage}&pagesize=${dataPerPage}&uappStudentId=${consUappIdValue}&studentId=${consStdValue}&universityId=${consUniValue}&uappPhoneId=${consPhnValue}&applicationStatusId=${applicationValue}&offerStatusId=${offerValue}&enrollmentId=${enrollValue}&intakeId=${intakeValue}&interviewId=${interviewValue}&elptId=${elptValue}&studentFinanceId=${financeValue}&orderId=${orderValue}&intakerangeid=${intakeRngValue}&consultantId=${consultantValue}`
    ).then((res) => {
      setLoading(false);
