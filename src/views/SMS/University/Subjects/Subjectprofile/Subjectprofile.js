...
const Subjectprofile = () => {
  ...
  useEffect(() => {
    const fetchedData = async () => {
      setLoading(true);
      try {
        await get(`Subject/Profile/${subjId}`).then((res) => {
            ...
            >
              ...
              <div className="subject-profile-content">
                <i
                  ...
                  onClick={backUrl}
                ></i>
                <div className="col-md-10 mx-auto mt-100px mb-20px">
                  <div className="d-flex align-items-center mb-10px">
                    <span className="subject-profile-title text-white">
                      {data?.name}
                    </span>

                    {permissions?.includes(permissionList?.Edit_Subjects) && (
                      <Link
                        to={`/add-University-course/${data?.universityId}/${subjId}`}
                      >
                        <i class="far fa-edit subject-profile-edit"></i>
                      </Link>
                    )}
                  </div>
                  ...
                  <div className="d-flex align-items-center subject-profile-head-text text-white mb-24px">
                    <img
                      src={locationIcon}
                      alt=""
                      className="subject-university-logo"
                    />
