...
const CopyUniversitySubjectDeliveryPattern = () => {
  ...
  useEffect(() => {
    ...
    get(`SubjectDeliveryPattern/GetBySubject/${newSubId}`).then((res) => {
      setPatternList(res);
    });
  }, [newSubId, success]);
  ...
  const redirectToNextPage = () => {
    history.push({
      pathname: `/copyAndAddUniversitySubjectTestScore/${id}/${subjId}/${newSubId}`,
    });
  };
  ...
  const handleDeleteDeliveryPattern = (id) => {
    ...
    setProgress1(true);
    const returnValue = remove(`SubjectDeliveryPattern/Delete/${id}`).then(
      (action) => {
  ...
  const onPreviousPage = () => {
    history.push(`/copyAndAddUniversitySubjectFee/${id}/${subjId}/${newSubId}`);
  };

  return (
    <div>
      <BreadCrumb
        title="subject Delivery Pattern"
                              ...
                              >
                                <ModalBody>
                                  <p>
                                    ...
                                    <b>{delPatternName}</b> ? Once Deleted it
                                    can't be Undone!
                                  </p>
                                </ModalBody>
                                ...
                                <ModalFooter>
                                  <Button
                                    ...
                                    color="danger"
                                    onClick={() =>
                                      handleDeleteDeliveryPattern(delPatternId)
                                    }
                                  >
...
export default CopyUniversitySubjectDeliveryPattern;
