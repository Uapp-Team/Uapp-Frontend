...
const TermsAndConditionAdmissionOfficer = () => {
  ...
  useEffect(() => {
    get(`AdmissionOfficerConscent/Get/${admissionOfficerId}`).then((res) => {
      setConscentData(res);
    });
  ...
  const sendEmail = () => {
    ...
    setProgress(true);
    put(`AdmissionOfficerConscent/SendEmail/${admissionOfficerId}`).then(
      (res) => {
        setProgress(false);
...
export default TermsAndConditionAdmissionOfficer;
