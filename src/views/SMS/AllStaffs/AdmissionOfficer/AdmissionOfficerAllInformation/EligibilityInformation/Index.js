...
const Index = () => {
  ...
  useEffect(() => {
    ...
    get(
      `AdmissionOfficerEligibility/GetAdmissionOfficerEligibility/${admissionOfficerId}`
    ).then((res) => {
      //
  ...
  const handleSubmit = (event) => {
    ...
    if (ValidForm()) {
      ...
      setProgress(true);
      post(`AdmissionOfficerEligibility/Eligibility`, subData).then((res) => {
        setProgress(false);
  ...
  return (
    <div>
      <BreadCrumb
        title="Admission Officer Eligibility Information"
        backTo={
        ...
        <CardBody>
          <TabContent activeTab={activetab}>
            <p className="section-title">
              Consultant Right to work Information{" "}
            </p>
            <TabPane tabId="5">
              <EligibilityForm
