...
const StaffContactInformation = () => {
  ...
  useEffect(() => {
    ...
    get(
      `ProviderComplianceAddress/GetByEmployeeId/${providerComplianceId}`
    ).then((res) => {
  ...
  const handleSubmit = (event) => {
    ...
    subData.append("complianceId", providerComplianceId);
    ...
    subData.append("employeeId", providerComplianceId);
    subData.append("id", addressData === null ? 0 : addressData?.id);

    if (ValidForm()) {
      setButtonStatus(true);
      setProgress(true);
      post("ProviderComplianceAddress/Create", subData).then((res) => {
        setProgress(false);
        setSuccess(!success);
  ...
  return (
    <div>
      <BreadCrumb
        title="Provider Compliance  Contact Information"
