...
const StaffPersonalInformation = () => {
  ...
  useEffect(() => {
    ...
    get(
      `ProviderCompliance/GetPersonalInformation/${providerComplianceId}`
    ).then((res) => {
  ...
  const handleSubmit = (event) => {
    ...
    if (IsFormValid()) {
      ...
      setProgress(true);
      put("ProviderCompliance/PersonalInformation", subdata).then((res) => {
        setProgress(false);
        setSuccess(!success);
  ...
  return (
    <div>
      <BreadCrumb
        title="Provider Compliance  Personal Information"
