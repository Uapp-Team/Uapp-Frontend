...
const Index = () => {
  ...
  useEffect(() => {
    ...
    get(`AdmissionManagerAddress/GetByManager/${admissionManagerId}`).then(
      (res) => {
  ...
  const handleSubmit = (event) => {
    ...
    if (ValidForm()) {
      ...
      post("AdmissionManagerAddress/Address", subData).then((res) => {
        setProgress(false);
        setSuccess(!success);
  ...
  return (
    <div>
      <BreadCrumb
        title="Admission Manager Contact Information"
        backTo={
      ...
      <Card>
        <CardBody>
          <TabContent activeTab={activetab}>
            <TabPane tabId="3">
              <Form onSubmit={handleSubmit} autocomplete="disable">
