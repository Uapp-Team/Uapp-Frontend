...
const Index = () => {
  ...
  useEffect(() => {
    get(`AdmissionManagerEmergency/GetByManager/${admissionManagerId}`).then(
      (res) => {
  ...
  const handleSubmit = (event) => {
    ...
    if (ValidateForm()) {
      ...
      post("AdmissionManagerEmergency/Address", subData).then((res) => {
        setProgress(false);
        setSuccess(!success);
  ...
  return (
    <div>
      <BreadCrumb
        title="Admission Manager Emergency Information"
        backTo={
