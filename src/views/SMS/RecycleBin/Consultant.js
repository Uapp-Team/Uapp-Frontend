...
const Consultant = () => {
  ...
  useEffect(() => {
    if (!isTyping) {
      Uget(
        `ConsultantBin?index=${currentPage}&size=${dataPerPage}&query=${searchStr}`
      ).then((res) => {
  ...
  return (
    <>
      <BreadCrumb title="Consultant" backTo="Recycle Bin" path="/recycle" />
                  ...
                  {dataList?.length > 0 &&
                    dataList?.map((item, i) => (
                      <tr key={i} className="border-buttom">
                        ...
                        <td>
                          <RecoveryButton
                            url={`consultantBin/Restore?consultantId=${item?.id}`}
                            success={success}
                            setSuccess={setSuccess}
                          />
                          {/* <DeleteButton
                        url={`Consultant/PermanentDelete/${item?.id}`}
                        success={success}
                        setSuccess={setSuccess}
                      /> */}
                        </td>
