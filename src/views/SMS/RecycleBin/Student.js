...
const Student = () => {
  ...
  useEffect(() => {
    if (!isTyping) {
      Uget(
        `StudentBin?index=${currentPage}&size=${dataPerPage}&query=${searchStr}`
      ).then((res) => {
  ...
  return (
    <>
      <BreadCrumb title="Student" backTo="Recycle Bin" path="/recycle" />
                  ...
                  {dataList?.length > 0 &&
                    dataList?.map((item, i) => (
                      <tr key={i} className="border-buttom">
                        ...
                        <td>
                          <RecoveryButton
                            url={`StudentBin/Restore?studentId=${item?.id}`}
                            success={success}
                            setSuccess={setSuccess}
                          />
                          {/* <DeleteButton
                        url={`StudentBin/Delete?studentId=${item?.id}`}
                        success={success}
                        setSuccess={setSuccess}
                      /> */}
                        </td>
