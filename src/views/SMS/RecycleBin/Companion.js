...
const Companion = () => {
  ...
  useEffect(() => {
    if (!isTyping) {
      Uget(
        `CompanionBin?index=${currentPage}&size=${dataPerPage}&query=${searchStr}`
      ).then((res) => {
  ...
  return (
    <>
      <BreadCrumb title="Companion" backTo="Recycle Bin" path="/recycle" />
                  ...
                  {dataList?.length > 0 &&
                    dataList?.map((item, i) => (
                      <tr key={i} className="border-buttom">
                        ...
                        <td>
                          <RecoveryButton
                            url={`CompanionBin/Restore?companionId=${item?.id}`}
                            success={success}
