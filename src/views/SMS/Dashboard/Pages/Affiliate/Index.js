...
import AffiliateRefer from "./AffiliateReffer/AffiliateRefer";
...
const Affiliate = () => {
  ...
  useEffect(() => {
    Uget(`Affiliate/get-dashboard-info/${userId}`).then((res) => {
      setCount(res?.data);
    });
    Uget(`Affiliate/get-active-status/${userId}`).then((res) => {
  ...
  const handleSubmit = (event) => {
    ...
    if (!email) {
      ...
      setProgress(true);
      put(
        `AffiliateTeamInvitation?affiliateid=${referenceId}&email=${email}`,
        subData
      ).then((action) => {
  ...
  return (
    <>
      ...
      <Row className="mb-4">
        <Col lg="9">
          <CountingCards count={count} setCount={setCount} />
