...
const UsersNoticeList = () => {
  ...
  useEffect(() => {
    get(
      `Notice/UserNotices?page=${currentPage}&pagesize = ${dataPerPage}`
    ).then((res) => {
  ...
  const redirectToConsultantProfile = (noticeId) => {
    history.push(`/view-notice/${noticeId}`);
  };
  ...
  return (
    <div>
      <BreadCrumb title="Notices" backTo="" path="/" />
      <Card>
        <CardBody>
          {/* <Row className="mb-3">
...
export default UsersNoticeList;
