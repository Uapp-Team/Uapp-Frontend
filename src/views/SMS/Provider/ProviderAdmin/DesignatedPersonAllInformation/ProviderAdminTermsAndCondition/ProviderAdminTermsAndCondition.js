...
const ProviderAdminTermsAndCondition = () => {
  ...
  useEffect(() => {
    get(`ProviderAdminConsent/Get/${providerAdminId}`).then((res) => {
      setConscentData(res);
    });
  ...
  const handleTerms = (event) => {
    ...
    subData.append("AdminId", providerAdminId);
    subData.append("IpAddress", apiInfo?.IPv4);
    setProgress(true);
    post("ProviderAdminConsent/Sign", subData).then((res) => {
      setProgress(false);
  ...
  const sendEmail = () => {
    ...
    setProgress(true);
    put(`ProviderAdminConsent/SendEmail/${providerAdminId}`).then((res) => {
      setProgress(false);
      ...
      <Card>
        <CardBody>
          ...
          <div className="d-flex justify-content-between">
            ...
            {/* <ButtonForFunction
              func={onGoUniProfile}
              className={"mr-1 mt-3 btn-primary"}
              name={"Go to Profile"}
              permission={6}
            /> */}
          </div>
...
export default ProviderAdminTermsAndCondition;
