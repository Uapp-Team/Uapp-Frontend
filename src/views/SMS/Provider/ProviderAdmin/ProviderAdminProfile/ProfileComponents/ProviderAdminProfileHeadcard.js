...
const ProviderAdminProfileHeadcard = ({ providerAdminId, userId }) => {
  ...
  useEffect(() => {
    if (providerAdminId !== undefined) {
      get(`ProviderAdminProfile/GetHead/${providerAdminId}`).then((res) => {
      ...
      get(`AccountStatusDD/Provider/${providerAdminId}`).then((res) => {
        setStatusType(res);
        setLoading(false);
      });
    } else {
      get(`ProviderAdminProfile/GetHead/${userId}`).then((res) => {
        console.log("ProfileHead", res);
        setHeadData(res);
        setProviderId(res?.providerId);
        setStatusLabel(res?.accountStatusName);
      ...
      get(`AccountStatusDD/Provider/${userId}`).then((res) => {
        setStatusType(res);
        setLoading(false);
      });
    }
  ...
  const selectStatusType = (label, value) => {
    ...
    const accountStatusData = {
      adminId: parseInt(providerAdminId),
      statusId: value,
    };

    put(`ProviderAdminInfo/UpdateAccountStatusChange`, accountStatusData).then(
      (res) => {
  ...
  const handleSubmitCoverPhoto = (event) => {
    ...
    subData.append("coverFile", FileList[0]?.originFileObj);
    ...
    if (FileList.length < 1) {
      ...
      put(`ProviderAdmin/UpdateCoverPhoto`, subData).then((res) => {
        setProgress(false);
        setButtonStatus(false);
  ...
  const handleSubmitProfilePhoto = (event) => {
    ...
    subData.append("profileFile", FileList1[0]?.originFileObj);
        ...
        <div className="uapp-employee-cover-image">
          <div
            ...
            style={{
              backgroundImage: `url(${
                headData?.coverImage
                  ? rootUrl + headData?.coverImage?.thumbnailUrl
                  : uapploader2
              })`,
            }}
          >
            ...
            <div className="uplode-cover-image">
              {permissions?.includes(permissionList.Update_AdmissionOfficer) ? (
                    ...
                    <div className="profile-pic">
                      {headData?.profileImage == null ? (
                        ...
                        <img
                          ...
                          src={rootUrl + headData?.profileImage?.thumbnailUrl}
                          alt="img-desc"
                        />
                      )}
            ...
            <Row>
              <Col md="7">
                      ...
                      <div className="mr-1">
                        {permissions?.includes(
                          ...
                          <Link
                            to={
                              providerAdminId == undefined
                                ? `/providerAdminGeneralInfo/${providerId}/${userId}`
                        ...
                        {/* <Link
                          to={
                            officerId == undefined
                              ...
                              : `/admissionOfficerDetailsInfo/${officerId}`
                          }
                        >
                ...
                <div className="d-flex">
                  ...
                  // to={
                  //   officerId !== undefined
                  //     ? `/applicationsFromConsultant/${officerId}`
                  //     : `/applicationsFromConsultant/${userId}`
                  // }
                  >
                  ...
                  <Link
                    // to={
                    //   officerId !== undefined
                    //     ? `/studentListByConsultant/${officerId}`
                    //     : `/studentListByConsultant/${userId}`
                    // }
              ...
              <Col md="5" className="mt-4">
                ...
                userTypeId == userTypes?.AdmissionOfficer ? (
                  <ul className="uapp-ul text-right">
                    ...
                    <li>
                      <span> Phone Number : {headData?.phone}</span>
                    </li>
                  </ul>
                ) : null}
              </Col>
...
export default ProviderAdminProfileHeadcard;
