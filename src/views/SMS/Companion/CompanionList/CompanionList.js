...
const CompanionList = () => {
  ...
  console.log(companionList, "companionList");
  ...
  const handleChecked = (e, i) => {
    ...
    setTableData(values);
    localStorage.setItem("ColumnCompanions", JSON.stringify(values));
  };
  ...
  const submitModalForm = (event) => {
    ...
    if (!/^(?=.*[a-zA-Z])(?=.*\d).{6,}$/.test(pass)) {
      ...
      setProgress(true);
      put(`Password/ChangePasswordForCompanion`, subData).then((res) => {
        setProgress(false);
