...
const ConsultantTermAndConditionDetails = () => {
  ...
  useEffect(() => {
    get(`ConsultantConscent/Get/${id}`).then((res) => {
      setConscentData(res);
    });
  ...
  const toggle = (tab) => {
    setActivetab(tab);
    if (tab == "1") {
      history.push(`/consultantDetails/${id}`);
    }

    if (tab == "2") {
      history.push(`/consultantTermsandConditions/${id}`);
    }
  };
  ...
  const handleTerms = (event) => {
    ...
    subData.append("ConsultantId", id);
  ...
  const sendEmail = () => {
    ...
    setProgress(true);
    put(`ConsultantConscent/SendEmail/${id}`).then((res) => {
      setProgress(false);
  ...
  const back = () => {
    history.push(`/consultantCommission/${id}`);
  };
  ...
  const backToConsultantList = () => {
    history.push(`/consultantProfile/${id}`);
  };

  const handleEdit = () => {
    history.push(`/consultantTermsInformation/${id}`);
  };
...
export default ConsultantTermAndConditionDetails;
