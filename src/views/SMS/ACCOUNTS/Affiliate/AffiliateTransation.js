...
const AffiliateTransation = () => {
  ...
  useEffect(() => {
    if (!isTyping) {
      get(
        `AffiliateTransactoin?&page=${currentPage}&pageSize=${dataPerPage}&status=${statusValue}&type=${typeValue}&fromdate=${fromDate}&todate=${toDate}&string=${searchStr}&affiliateid=${affiliateValue}`
      ).then((res) => {
        setData(res?.models);
