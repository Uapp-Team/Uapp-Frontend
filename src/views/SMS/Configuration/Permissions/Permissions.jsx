...
const Permissions = (props) => {
  ...
  useEffect(() => {
    get(`Permission/Index`).then((data) => {
  ...
  const handleSubmit = (event) => {
    ...
    post(`Permission/Create`, subdata).then((action) => {
      setSuccess(!success);
      setModalOpen(false);
      addToast(action, {
        appearance:
          action == "Permission Added Successfully" ? "success" : "error",
        autoDismiss: true,
      });
      setPermissionName("");
      setSelected("");
  ...
  const handleDeletePermission = (id) => {
    remove(`Permission/Delete/${id}`).then((action) => {
      setDeleteModal(false);
      setSuccess(!success);
      addToast(action, {
        appearance:
          action == "Permission Deleted Successfully" ? "warning" : "error",
        autoDismiss: true,
      });
  ...
  const handleUpdateSubmit = () => {
    ...
    put(`Permission/Update`, subData).then((action) => {
      setSuccess(!success);
      setModalOpen(false);
      addToast(action, {
            ...
            <tbody>
              {permissionList?.map((permission, i) => (
                <tr>
                  ...
                  <td>
                    <Button
                      onClick={() => toggleDanger(permission)}
                      color="danger"
                    ...
                    >
                      <ModalBody>
                        <p>
                          ...
                          {localStorage.getItem("delPermissionName")} ? Once
                        ...
                        <Button
                          onClick={() =>
                            handleDeletePermission(
                              localStorage.getItem("delPermissionId")
                            )
                          }
                          color="danger"
                        >
