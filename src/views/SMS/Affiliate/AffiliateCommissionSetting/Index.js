...
import AffiliateCommissionForm from "./Component/AffiliateCommissionForm";
import AffiliateCommissionList from "./Component/AffiliateCommissionList";
...
const DistributionLevelSetting = () => {
  ...
  useEffect(() => {
    Uget(`AffiliateCommissionSetting/Index`).then((res) => {
  ...
  const toggleUpdate = (data) => {
    ...
    setNameError("");
    setPercentError("");
    Uget(`AffiliateCommissionSetting/${data?.id}`).then((res) => {
      console.log(res);
      setData(res?.data);
  ...
  return (
    <div>
      <BreadCrumb title="Affiliate Commission Settings" backTo="" path="/" />
      <div>
        <Card>
          <CardBody>
            {loading ? (
