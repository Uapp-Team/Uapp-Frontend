...
const ConsultantBankDetails = () => {
  ...
  useEffect(() => {
    Uget(`AffiliateBankDetails/get-by/${affiliateId}`).then((res) => {
  ...
  const goForward = () => {
    if (userTypeId === userTypes?.Affiliate.toString()) {
  ...
  const handleSubmitUpdate = (event) => {
    ...
    if (ValidateForm()) {
      ...
      setProgress(true);
      post("AffiliateBankDetails/save", subData).then((res) => {
        setProgress(false);
  ...
  const handleDeletePermission = () => {
    setButtonStatus(true);
    setProgress(true);
    Uremove(`AffiliateBankDetails/Delete/${deleteData?.id}`).then((res) => {
      setProgress(false);
      setButtonStatus(false);
      addToast(res?.data?.title, {
        appearance: "error",
