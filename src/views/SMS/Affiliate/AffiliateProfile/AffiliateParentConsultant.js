import React, { useEffect, useState } from "react";
import { Card, Col, Row } from "reactstrap";
import { useHistory } from "react-router-dom";
import profileImage from "../../../../assets/img/profile/user-uploads/user-07.jpg";
import get from "../../../../helpers/get";
import { rootUrl } from "../../../../constants/constants";
import { userTypes } from "../../../../constants/userTypeConstant";

const AffiliateParentConsultant = ({ affiliateProfileData, affiliateId }) => {
  const [consultantData, setConsultantData] = useState({});
  // const [success, setSuccess] = useState(false);
  const userTypeId = localStorage.getItem("userType");
  const history = useHistory();

  const adminRedirectToParentConsultantProfile = () => {
    history
      .push
      // `/consultantProfile/${affiliateProfileData?.data?.parent?.id}`
      ();
  };
  const redirectToParentConsultantProfile = () => {
    // history.push(`/parentConsultantProfile/${consultantData?.id}`);
  };

  return (
    <>
      {affiliateProfileData?.data?.parentConsultant === null ? null : (
        <Card className="p-4">
          <div className="d-flex justify-between-start">
            <div className="pr-3">
              {affiliateProfileData?.data?.parentConsultant?.profilePic ==
              null ? (
                <img
                  src={profileImage}
                  alt="profile_img"
                  style={{
                    width: "50px",
                    height: "50px",
                    borderRadius: "50px",
                  }}
                />
              ) : (
                <img
                  src={
                    rootUrl +
                    affiliateProfileData?.data?.parentConsultant?.profilePic
                      ?.fileUrl
                  }
                  alt="profile_img"
                  style={{
                    width: "50px",
                    height: "50px",
                    borderRadius: "50px",
                  }}
                />
              )}
            </div>
            <div>
              {" "}
              <h5>
                {userTypeId === userTypes?.SystemAdmin.toString() ||
                userTypeId === userTypes?.Admin.toString() ? (
                  <span
                    onClick={adminRedirectToParentConsultantProfile}
                    // style={{ cursor: "pointer" }}
                  >
                    {affiliateProfileData?.data?.parentConsultant?.name}
                  </span>
                ) : (
                  <span
                    onClick={redirectToParentConsultantProfile}
                    // style={{ cursor: "pointer" }}
                  >
                    {affiliateProfileData?.data?.parentConsultant?.name}
                  </span>
                )}
              </h5>
              {/* {userTypeId === userTypes?.Consultant.toString() ? null : (
                <p className="text-gray-70">
                  {consultantData?.parentConsultant}{" "}
                </p>
              )} */}
              <p> Parent Consultant</p>
            </div>
          </div>

          <Row>
            <Col>
              <ul className="uapp-ul">
                {affiliateProfileData?.data?.parentConsultant?.email ===
                null ? null : (
                  <li>
                    <i className="far fa-envelope pr-2"></i>
                    {affiliateProfileData?.data?.parentConsultant?.email}
                  </li>
                )}

                {affiliateProfileData?.data?.parentConsultant?.phone ===
                null ? null : (
                  <li>
                    <i className="fas fa-phone pr-2"></i>
                    {affiliateProfileData?.data?.parentConsultant?.phone &&
                    affiliateProfileData?.data?.parentConsultant?.phone.includes(
                      "+"
                    )
                      ? affiliateProfileData?.data?.parentConsultant?.phone
                      : affiliateProfileData?.data?.parentConsultant?.phone &&
                        !affiliateProfileData?.data?.parentConsultant?.phone.includes(
                          "+"
                        )
                      ? "+" +
                        affiliateProfileData?.data?.parentConsultant?.phone
                      : null}
                  </li>
                )}
              </ul>
            </Col>
          </Row>
        </Card>
      )}
    </>
  );
};

export default AffiliateParentConsultant;
