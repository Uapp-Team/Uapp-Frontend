...
const MyTeamList = () => {
  ...
  useEffect(() => {
    if (!isTyping) {
      Uget(
        `Affiliate/get-team-members/?&affiliateId=${affiliateId}&searchstring=${searchStr}&page=${currentPage}&pageSize=${dataPerPage}`
      ).then((res) => {
        console.log(res);
  ...
  }, [currentPage, dataPerPage, isTyping, affiliateId, searchStr]);
        ...
        <CardBody>
          ...
          <Row className="mb-3">
            <Col lg="5" md="5" sm="12" xs="12" style={{ marginBottom: "10px" }}>
              ...
              <LinkButton
                url={`/affiliate-registrationby/${affiliateId}`}
