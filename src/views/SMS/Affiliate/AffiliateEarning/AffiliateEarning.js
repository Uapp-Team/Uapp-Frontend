...
import AffInvite from "./AffInvite";
...
const AffiliateEarning = () => {
  ...
  useEffect(() => {
    get(`AffiliateTransactoin/Balance/${referenceId}`).then((res) => {
      setBalance(res);
    });
    if (!isTyping) {
      get(
        `AffiliateTransactoin?&page=${currentPage}&pageSize=${dataPerPage}&status=${statusValue}&type=${typeValue}&fromdate=${fromDate}&todate=${toDate}&string=${searchStr}`
      ).then((res) => {
        setData(res?.models);
  ...
  const handleSubmit = (e) => {
    ...
    if (!amount) {
      ...
      put(
        `AffiliateTransactoin/Createwithdrawrequest?affiliateid=${referenceId}&amount=${amount}`
      ).then((res) => {
        setProgress(false);
