...
const MyTeamList = () => {
  ...
  const [affiliateTeamList, setAffiliateTeamList] = useState([]);
  ...
  useEffect(() => {
    Uget(`Affiliate/get-active-status/${referenceId}`).then((res) => {
  ...
  useEffect(() => {
    get(`AffiliateTeamInvitation?&affiliateid=${referenceId}`).then(
      (action) => {
  ...
  useEffect(() => {
    if (!isTyping) {
      Uget(
        `Affiliate/get-team-members/?&affiliateId=${referenceId}&searchstring=${searchStr}&page=${currentPage}&pageSize=${dataPerPage}`
      ).then((res) => {
        console.log(res);
        setAffiliateTeamList(res?.items);
  ...
  const handleAffiliateSubmit = (event) => {
    ...
    if (!emailAffiliate) {
      ...
      setProgress(true);
      put(
        `AffiliateTeamInvitation?affiliateid=${referenceId}&email=${emailAffiliate}`,
        subData
      ).then((action) => {
        ...
        <CardBody>
          ...
          <Row className="mb-3">
            <Col lg="5" md="5" sm="12" xs="12" style={{ marginBottom: "10px" }}>
              ...
              {active && (
                <LinkButton
                  url={`affiliate-registrationby/${referenceId}`}
                ...
                <>
                  {loading ? (
                    ...
                    <AffiliateTable
                      ...
                      userTypes={userTypes}
                      affiliateTeamList={affiliateTeamList}
                      history={history}
                ...
                <Form onSubmit={handleAffiliateSubmit}>
                  ...
                  <FormGroup row className="has-icon-left position-relative">
                    ...
                    <Col md="8">
                      ...
                      />
                      <span className="text-danger">{emailAffiliateError}</span>
                    </Col>
                  </FormGroup>
